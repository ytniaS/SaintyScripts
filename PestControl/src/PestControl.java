...
public class PestControl extends Script {
	...
	private static final int PEST_CONTROL_WORLD = 344; // currently only 1 pest control world that's feasible to use
	...
	//area to hopefully prevent pathing around the outside where it may get stuck on searching for region
	...
	public int poll() {
		...
		if (me == null) {return 300;}
		int region = me.getRegionID();
		//detect game end by going from game -> lobby regions
		...
		if (region == REGION_GAME) {
			//small delay once the game starts to ensure everything is loaded
			if (!inGame) {
				inGame = true;
			...
			if (!instanceSettled) {
				if (System.currentTimeMillis() < instanceResolveUntil) {return 300;}
				instanceSettled = true;
			}
			handleGame();
		}
		if (region == REGION_LOBBY) {
			if (ensureCorrectWorld()) {return 600;}
			handleLobby();
		}
		return random(40, 80);
	}
	...
	private void handleGame() {
		...
		//if we path outside the 'safe' zone, walk back
		if (!SAFE_COMBAT_AREA.contains(me)) {
			attacking = false;
			targetOverlay = null;
			if (recoveringToCombat) {return;}
			if (System.currentTimeMillis() - lastRecoverAttempt < RECOVER_COOLDOWN_MS) {return;}
		...
		if (attacking && targetOverlay != null && targetOverlay.isVisible()) {return;}
		attacking = false;
		targetOverlay = null;
		attackNpc();
	}
	...
	private boolean attackNpc() {
		...
		for (WorldPosition npc : targets) {
			...
			if (!getWidgetManager().insideGameScreen(poly, Collections.emptyList())) {continue;}
	...
	private boolean tryWalkInsideCombatArea(WorldPosition target) {
		if (!instanceSettled || target == null) {return false;}
		if (!COMBAT_AREA.contains(target)) {target = randomIn(VOID_KNIGHT_RECT);}
		long now = System.currentTimeMillis();
		//prevent spam walking
		if (now - lastWalkAt < 1000) {return true;}
	...
	private void handleLobby() {
		...
		if (boardingInProgress) {
			if (System.currentTimeMillis() - boardingStart > BOARDING_MAX_MS) {boardingInProgress = false;}
			return;
		}
		if (awaitingResult) {
			checkGameResult();
		...
		if (System.currentTimeMillis() - lastBoardClick < BOARD_CLICK_COOLDOWN_MS) {return;}
		Polygon cube = getSceneProjector().getTileCube(selectedBoat.plank, 60);
		if (cube == null) {return;}
		Polygon click = cube.getResized(0.35);
		if (!getWidgetManager().insideGameScreen(click, Collections.emptyList())) {return;}
	...
	private boolean ensureCorrectWorld() {
		...
		if (current != null && current == PEST_CONTROL_WORLD) {return false;}
	...
	private boolean tryWalkToLobby(WorldPosition target) {
		...
		long now = System.currentTimeMillis();
		if (now - lastWalkAt < 1200) {return true;}
		WalkConfig cfg = new WalkConfig.Builder()
				.tileRandomisationRadius(1)
				.breakDistance(1)
				.build();
	...
	private boolean isOnBoat() {
		...
		for (RSObject o : decks) {
			...
			if (p != null && me.distanceTo(p) <= 2) {return true;}
		}
		return false;
	}
	...
	private void checkGameResult() {
		...
		//checks the dialogue popup for commendations
		...
		//if the game fails there are no commendations in the dialogue
		if (foundWin ||
				System.currentTimeMillis() - awaitingResultStart > RESULT_OBSERVE_TIME) {
			if (!winDetected) {gamesLost++;}
	...
	private void trackDeath() {
		...
		if (hp == null) {return;}
		if (lastHp != null) {
			if (lastHp > 0 && hp == 0) {deathFlag = true;}
