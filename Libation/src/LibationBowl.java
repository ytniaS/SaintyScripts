...
public class LibationBowl extends Script {
	...
	//rectangles for quetzal traversal so it doesn't always click the same pixel
	...
	private void blessWine() {
		...
		if (!hasNormal && !hasSunfire) {return;}
	...
	private void acquireMoreWine(ItemGroupResult invSnapshot) {
		//  Always handle empty jugs first — this preempts shopping
		...
		if (!getFinger().tap(resized, "Trade")) {return;}
	...
	private void depositEmptyJugs() {
		...
		if (jugs == null || !jugs.contains(EMPTY_JUG)) {return;}
		if (inRegion(REGION_TEOMAT)) {
