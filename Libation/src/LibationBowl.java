...
public class LibationBowl extends Script {
    ...
    private static final long TELEMETRY_INTERVAL_MS = 60_000;
    private long lastJugDepositAt = 0;
    private static final long JUG_BANK_COOLDOWN_MS = 1200;
    ...
    private com.osmb.script.libation.WineShopInterface wineShop;
    ...
    public void onStart() {
        ...
        wineShop = new com.osmb.script.libation.WineShopInterface(this);
    ...
    private boolean bankJugsAndWithdrawWineIfPossible() {
        ...
        if (!pollFramesUntil(() -> getWidgetManager().getBank().isVisible(), 5000)) {
            return true;
        }

        // deposit empty jugs
        if (hasJugs) {
            ItemGroupResult jugs =
                    getWidgetManager().getInventory().search(Collections.singleton(EMPTY_JUG));
            if (jugs != null && jugs.contains(EMPTY_JUG)) {
                getWidgetManager().getBank()
                        .deposit(EMPTY_JUG, jugs.getAmount(EMPTY_JUG));
            }
        }
